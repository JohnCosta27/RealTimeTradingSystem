...
As mentioned before, I am using docker throughout my application and docker compose to allow for easy deployment of various containers. The Dockerfile for the services are as follows:
...
This dockerfile copies the content of the authentication service onto the container, as well as copying the shared libraries so they can be bundled in with the final executable. Finally we compile the program, expose the port and execute the compiled executable. I am doing the same thing for the Hub and Brain service, just with a different folder, therefore I won't include them.
...
Furthermore, one of the most popular and advances document databases available is MongoDb ~\cite{mongodb}, which uses JSON as a method of storage, and has support for data sharding, and (as most document databases do), does not require a schema, giving a lot of flexibility to the developer. However, as shown in this paper ~\cite{mongodb_vs_postgres}, MongoDb performs worst than PostgreSQL in almost all performance tests, often by some great magnitude. \\
